 ...
 * @author Fpt
 */
@WebServlet(name = "AddCourseWork", urlPatterns = {"/AddCourseWork"})
public class AddCourseWork extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        boolean check = new CourseWorkDAO().checkCourseWork(courseId, yearmaking);
        if(check == true)
        {
            request.setAttribute("coureIdErr", "Has been exist CourseWork for this year");
            out.println("<html><body onload=\"alert('Has been exist CourseWork for this year')\"></body></html>");
            bl = false;
        }
        ...
        if (bl) {
            ...
            try {
                pstm = con.prepareStatement("insert into CourseWork(courseId,departmentId,courseLeader,courserModerator,createDate,yearMaking,semester,studentCount,courserUnits,costUnit,ishadCMR)" + " values (?,?,?,?,?,?,?,?,?,?,?)");
                ...
                pstm.setBoolean(11, ishadCMR);

                int result = pstm.executeUpdate();
                if (result > 0) {
                    kq = true;
                    request.setAttribute("insertStatus", "Insert successfull for Course id:" + courseId);
                    ...
                    //ko thanhcong
                    request.setAttribute("insertStatus", "Insert failed for Course id:" + courseId);
            ...
            request.getRequestDispatcher("PreAddCourseWork").forward(request, response);
        }
    }
